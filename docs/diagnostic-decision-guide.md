...
## 최초 샘플 기반 워크북 보완 (2026-09-21)

`channeldive-swtajuu8.manus.space/practice`를 모바일 390px에서 확인했다. 참고한 핵심은 전체 목차와 진행률을 먼저 보여주고, 각 장에서 `개념 → 판단 기준 → 바로 실습 → 결과물`로 이어지며 마지막에 답변을 모으는 구조다. 147개 입력을 한 페이지에 모두 노출하고 브라우저 저장을 지원하지 않는 부분은 자사몰 진단에 그대로 적용하지 않았다.

- 인트로에서 고객이 찾아와 다시 찾기까지의 6챕터와 챕터별 산출물을 먼저 보여준다.
- 기존 정밀 27문항을 `성장 워크북` 주 경로로 바꾸고, 빠른 10문항은 보조 경로로 유지한다.
- 각 챕터의 질문 전에 볼 관점, AI가 정리할 일, 대표가 판단할 일, 완료 산출물을 보여준다.
- 3장과 5장 완료 시 결과를 기다리지 않고 문의하거나 다음 챕터를 계속할 수 있는 체크포인트를 둔다.
- 기존 문항 ID·순서·응답 타입·점수 방향·결과 저장·공유는 유지한다.
- Excel 원본의 약속·감사·고객 학습 관점은 기존 질문·점수·과거 결과의 의미를 바꾸지 않고 5·6장 개념 안내와 판단 기준에 반영했다.
- 중간 CTA는 `workbook_checkpoint_3`, `workbook_checkpoint_5` ref와 `workbook_checkpoint_cta` 이벤트로 최종 결과 CTA와 구분한다.

검증은 전체 `npm test` 34개 파일 / 265개 테스트, `npx tsc --noEmit`, Next.js 14.2.35 프로덕션 빌드, `git diff --check`를 통과했다. 로컬 모바일 브라우저에서 인트로 → 6챕터 개념 카드 → 27문항 → 3·5장 중간 문의 → ICP → Vision → 최종 결과까지 완료했고, 최종 근거·AI/대표 역할·카카오 문의가 유지되는 것을 확인했다.
